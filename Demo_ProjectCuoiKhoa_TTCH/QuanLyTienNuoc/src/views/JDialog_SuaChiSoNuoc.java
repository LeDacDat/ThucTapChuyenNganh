 ...
 * @author Admin
 */
public class JDialog_SuaChiSoNuoc extends javax.swing.JDialog {
    ...
    private final ChiSoNuoc chiSoNuoc;
    
    public JDialog_SuaChiSoNuoc(KhachHang khachHang, ChiSoNuoc chiSoNuoc) {
    ...
    private void initView(){
        ...
        jTextField_MaThang.setText(chiSoNuoc.getThangID() + "");
        jTextField_HoTen.setText(khachHang.getHoTen());
        jTextField_SoDienThoai.setText(khachHang.getSoDienThoai());
        
        ChiSoNuoc chiSoNuocFirst = chiSoNuocServiceImpl.getFirstChiSoNuoc(khachHang.getMaKH());
        
        // neu chinh sua chi so Nuoc dau tien
        if (chiSoNuocFirst.getThangID() == chiSoNuoc.getThangID()){
            // cho phep chinh sua chi so dau tien
            jTextField_ChiSoCu.setEditable(true);
        }
        
        jTextField_ChiSoCu.setText(chiSoNuoc.getChiSoCu() + "");
    ...
    private void updateChiSoNuoc(){
        ChiSoNuoc chiSoNuocUpdate_1 = new ChiSoNuoc(chiSoNuoc.getId(), chiSoNuoc.getMaKH(), chiSoNuoc.getThangID(),
                Integer.parseInt(jTextField_ChiSoCu.getText()),
                Integer.parseInt(jTextField_ChiSoMoi.getText()), new Date());

        ChiSoNuoc chiSoNuocUpdate_2 = chiSoNuocServiceImpl.getChiSoNuoc(khachHang.getMaKH(), chiSoNuoc.getThangID() + 1);
        if (chiSoNuocUpdate_2 != null) chiSoNuocUpdate_2.setChiSoCu(Integer.parseInt(jTextField_ChiSoMoi.getText()));

        if (chiSoNuocServiceImpl.createORUpdate(chiSoNuocUpdate_1) != 0
                && chiSoNuocServiceImpl.createORUpdate(chiSoNuocUpdate_2) != 0){
    ...
    private boolean validChiSo(String chiSoCu_text, String chiSoMoi_text){
        try{
            ...
            ChiSoNuoc chiSoNuocSau = chiSoNuocServiceImpl.getChiSoNuoc(khachHang.getMaKH(), chiSoNuoc.getThangID() + 1);
    ...
    private void initComponents() {
        ...
        jPanel_Data.setBorder(javax.swing.BorderFactory.createTitledBorder(null, "Sửa chỉ số nước", javax.swing.border.TitledBorder.DEFAULT_JUSTIFICATION, javax.swing.border.TitledBorder.DEFAULT_POSITION, new java.awt.Font("Tahoma", 0, 14))); // NOI18N
